...
function dsrcreateGraphsFromTemplates($name, $orderby="service", $sumgraphs=false,$aggregate=false) {
	global $graphTemplate, $COLORS, $graphs;
	global $graphs, $metrics, $templatecolors, $graphtitle, $graphhost, $graphservice, $colors;
	global $hostpattern, $servicepattern, $graphtitle, $sectiontitle,$graphalias;
	global $metricserieslist, $metricpatterns,$metricaliases,$metricfunctions;
...
//first metricspatterns hit. we local it.

//mmmmm...
		// IF metricpatterns is set, use that! ELSE use servicepattern to find them
...
echo "<pre>";
//print_r($graphTemplate[$name]['metricpatterns']);
print_r($metricpatterns);
//print_r($graphTemplate[$name]['metric']["*"]['pattern']);
echo "</pre>";
//exit;
